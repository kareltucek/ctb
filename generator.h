...
{
  if(op->is(fINPUT))
    w.print("$1 $2 = $3;", op->get_type_string(), W().print(acces, i*myout), op->get_op_string(), get_inout_pos(), ARG(1), ARG(2), ARG(3));
  else if(op->is(fOUTPUT))
    w.print("        $3;", op->get_type_string(), W().print(acces, i*myout), op->get_op_string(), get_inout_pos(), ARG(1), ARG(2), ARG(3));
  else
    w.print("$1 $2 = $3;", op->get_type_string(), W().print(acces, i*myout), op->get_op_string(), 0              , ARG(1), ARG(2), ARG(3));
}
}
    ...
    for(auto itr : acces_map)
    {
      ...
      if(op->get_conv_string(itr.first, width, c1, c2, t))
      {
        ...
        if(itr.first > width)
        {
          ...
          {
            w.print("$1 $2 = $3;", t, W().print(acces, i*itr.first      ), c1, get_inout_pos(), W().print(itr.second, i*itr.first), "", "");
            w.print("$1 $2 = $3;", t, W().print(acces, i*itr.first+width), c2, get_inout_pos(), W().print(itr.second, i*itr.first), "", "");
          }
          return acces;
        }
        else
        {
          ...
          {
            w.print("$1 $2 = $3;", t, W().print(acces, i*width          ), c1, get_inout_pos(), W().print(itr.second, i*width),  W().print(itr.second, i*width+itr.first), "", "");
          }
          return acces;
        }
      }
