...
\end{longtable}
%\end{center}
\myendfig{sse_boolean_conversion}{Width conversion suitable for booleans.}

Notice that to perform this split, a nontrivial data reordering was neccessary. A simpler approach would simply select every second element by an \emph{and} mask and then then shift the second vector. Showing this by a pseudo-vector C operation, we would do:
...
Width conversions (as described above) may be implemented by means of a set of \ttt{shuffle} operations. We provide an excerpt dedicated to shuffle macros from the SSE documentation \cite{TODO} to portray its semantics: 

\myloosequote{ The Streaming SIMD Extensions (SSE) provide a macro function to help create constants that describe shuffle operations. The macro takes four small integers (in the range of 0 to 3) and combines them into an 8-bit immediate value used by the SHUFPS instruction. You can view the four integers as selectors for choosing which two words from the first input operand and which two words from the second are to be put into the result word. }{TODO}
