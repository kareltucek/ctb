...
We implement the second recursive call using a queue to prevent infinite loops on call stack. The actual implementation also handles crawling on a defined subset of graph layers passed by the \ttt{l} argument.
...
\begin{itemize}
  \item The \ttt{lastpass} variable is used in combination with the \ttt{passid} variable as an auxiliary mark which denotes whether a node was visited. The \ttt{passid} ensures that a new mark which was not yet used is used.
...
\end{code}
\myendfig{factoralgorder1}{Factorisation -- construction of translation map for acyclic graphs.}
...
foreach(node n in G)
  n->classid = translation[n->classid];
\end{code}
\myendfig{factoralgorder2}{Factorisation -- construction of translation map for acyclic graphs.}
