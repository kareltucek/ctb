...
namespace ctb
{
   ...
   *
   * The input interface consists of the instruction_table::add_type and the instruction_table::add_operation create new operation/type with specified id and return a reference to it, which then can be used to fill in code snippets for realization of the code/snippets.
   *
   ...
   * */
   ...
   class instruction_table //provides transition from opcode to abstract operation
   {
     ...
     private:
       ...
       bool sig_acceptable( const vector<tid_t>& pattern, const vector<tid_t>& string) const; 
       void init();
     public:
       ...
       template<typename C> const expansion_t& find_expansion(const C& cont, const vector<tid_t>& sig, bool* ambiguous = NULL) const; 
       /*API*/ const expansion_cont_t& find_expansion(const string& transformer, const string& expansion) const; 
       /*API*/ const expansion_t& find_expansion(const string& transformer, const string& expansion, const vector<tid_t>& types) const; 
       ...
       /*IAPI*/ type_t& add_type(typename T::tid_t t, int bitwidth = 0) ;
       instruction_table();
       void clear(bool tags = false, bool reinit = true) ;
       bool empty();
       ~instruction_table();
   } ;
   ...
   template <class T>
     instruction_table<T>::instruction_table() : instab(), typetab(), exptab(), taglists()
   {
     init();
   }
   ...
   template <class T>
     typename instruction_table<T>::type_t& instruction_table<T>::add_type(typename T::tid_t t, int bitwidth)
     {
   ...
   template <class T>
   void  instruction_table<T>::init()  
     {
       add_type("_notype");
       add_operation("_noop", "_notype", {}, fNOOP);
     }
   ...
   template <class T>
     bool instruction_table<T>::sig_acceptable( const vector<tid_t>& pattern, const vector<tid_t>& signature) const
   {
     if(pattern.size() != signature.size())
       return false;
     for(int i = 0; i < pattern.size(); i++)
     {
       if(pattern[i] != signature[i] && signature[i] != "")
         return false;
     }
     return true;
   }
   ...
   template <class T>
     template<typename C>
     const typename instruction_table<T>::expansion_t& instruction_table<T>::find_expansion(const C& exps, const vector<tid_t>& types, bool* ambiguous) const
     {
       ...
       if(acceptable.size() > 1)
       {
         if(ambiguous != NULL)
           *ambiguous = true;
         warn(string("general expansion search yields a random result for a search with ambiguous results. Required type signature is  ") + ctb::l_to_string(types) );
       }
       if(acceptable.size() >= 1)
         return *(acceptable.front());

       error(string("expansion for the following signature not found: ") + "'" + ctb::l_to_string(types) + "'");
       throw "unreachable throw preventing no-return-value warnings";
     }

   template <class T>
     const typename instruction_table<T>::expansion_cont_t& instruction_table<T>::find_expansion(const string& transformer, const string& name) const
     {
       auto it1 = exptab.find(transformer);
       if(it1 != exptab.end())
       {
       ...
       error(string("expansion container not found: ") + "'" + transformer + ":" + name + "'");
       throw "unreachable throw preventing no-return-value warnings";
     }

   template <class T>
     const typename instruction_table<T>::expansion_t& instruction_table<T>::find_expansion(const string& transformer, const string& name, const vector<tid_t>& types) const
     {
       ...
       {
         bool ambig = false;
         const auto* res = &find_expansion(exps, types, &ambig);
         if(ambig)
           warn(string("note: ambiguous search initiated for an expansion of ") + transformer + ":" + name);
         return *res;
       }
       RETHROW(string("search in the global expansion search table invoked for a specific transformer: ") + transformer + ":" + name);
       throw "unreachable throw preventing no-return-value warnings";
     }
   ...
   template <class T>
     void instruction_table<T>::clear(bool tags, bool reinit)
     {
       for(auto d : typetab)
       {
         delete d.second;
